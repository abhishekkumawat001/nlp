...
class RNN_LM(nn.Module):
    ...
    def __init__(self, vocab_size, emb_size, hidden_size, num_layers=1, dropout=0.3):
        super(RNN_LM, self).__init__()
        # YOUR CODE HERE
        self.embedding = nn.Embedding(vocab_size, emb_size) #define embedding layer
        self.rnn = nn.RNN(emb_size, hidden_size, num_layers,   dropout=dropout, batch_first=True) #define RNN layer
    ...
    def get_most_likely_chars(self, sequence, k):
        ...
        try:
            ...
            while len(top_k_chars) < k:
                top_k_chars.append('?')
            # # Handle the case where top_k_indices might still be a nested list
            # if isinstance(top_k_indices, list) and isinstance(top_k_indices[0], list):
            #     top_k_indices = top_k_indices[0]  # Extract the inner list
...
hidden_size = 10        # Hidden layer dimension
